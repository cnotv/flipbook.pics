...
describe("Loader Component - Integration Tests", () => {
  ...
  describe("Time Formatting", () => {
    ...
    it("should calculate current time based on frame position", async () => {
      ...
      // Should calculate proportional time (frame 1 of 3 frames in 4 seconds = 1.333 seconds)
      const currentTime = vm.currentTime;
      expect(currentTime).toBe("0:01.333"); // Corrected expectation
    });
  ...
  describe("Video Loading and Frame Generation", () => {
    it("should generate frames when a video is loaded", async () => {
      ...
      // Load video
      await vm.onVideoUpload(mockEvent);
      await nextTick();
    ...
    it("should generate frames when sample video is loaded", async () => {
      ...
      // Load sample video (this will use the mocked video element)
      vm.loadSampleVideo();
      await nextTick();
  ...
  describe("Automatic Toggle Play After Loading", () => {
    ...
    it("should loop frames during playback", async () => {
      ...
      // Simulate frame advance by calling advanceFrame directly
      vm.advanceFrame();
      await nextTick();
  ...
  describe("Time Formatting and Display", () => {
    ...
    it("should calculate current and total time correctly", async () => {
      ...
      expect(vm.currentTime).toBe("0:01.333"); // Corrected expectation: (1/3) * 4 = 1.333 seconds
  ...
  describe("Frame Generation Deterministic Behavior", () => {
    it("should generate frames using deterministic utilities", async () => {
      ...
      // Should use the mocked utilities
      const { calculateTargetFrameTimes } = await import("../../helper/frameGeneration");
      expect(calculateTargetFrameTimes).toHaveBeenCalledWith(2.0, 30);
    });
  });
});
